...
export function DonationModal() {
  ...
  const handleMobileMoneySubmit = async (e?: React.FormEvent) => {
    ...
    toast.error(`${providerLabel} payments are currently under configuration. This payment option is not yet available for live donations. Please try another available payment method.`);
  };
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          ...
          {!done && step === 2 && (
            <div className="px-6 py-4 space-y-4 animate-fade-in-up">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                ]).map(({ id, circle, bg, label, span, isConfiguring }) => (
                  <button
                    key={id}
                    type="button"
                    onClick={() => setMethod(id)}
                    title={isConfiguring ? "This payment method is currently being configured and is not available for live donations." : undefined}
              ...
              {(method === 'mtn' || method === 'airtel') && (
                <div className="p-3 bg-amber-50 border border-amber-200/80 rounded-xl flex items-start gap-2.5 text-left animate-fade-in-up">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[11px] text-amber-800 leading-relaxed">
                      This payment method is currently being configured and is not available for live donations.
                    </p>
                  </div>
                </div>
              )}
          ...
          {!done && step === 3 && (
            <div>
              ...
              {/* ── MTN / AIRTEL UNDER CONFIGURATION ──────────────── */}
              {(method === 'mtn' || method === 'airtel') && (
                <div className="space-y-0 animate-fade-in-up">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        ...
                        style={{ backgroundColor: method === 'mtn' ? 'rgba(0,0,0,0.12)' : 'rgba(255,255,255,0.18)' }}
                      >
                        {method === 'mtn'
                          ? <span className="text-[12px] font-black text-black leading-none tracking-tight">MTN</span>
                          : <span className="text-[20px] font-black text-white leading-none">A</span>}
                      </div>
                      <div>
                        <p className="text-xs font-black tracking-wide" style={{ color: method === 'mtn' ? '#1a1a1a' : 'white' }}>
                          {method === 'mtn' ? 'MTN Mobile Money' : 'Airtel Money'}
                        </p>
                        <p className="text-[10px] font-medium" style={{ color: method === 'mtn' ? 'rgba(0,0,0,0.6)' : 'rgba(255,255,255,0.8)' }}>
                          Payment Service Under Configuration
                        </p>
                      </div>
                    </div>
                  ...
                  {/* Body */}
                  <div className="px-6 pt-6 pb-6 space-y-5 text-left">
                    {/* Amount card */}
                    <div className="rounded-xl px-5 py-3.5 flex items-center justify-between bg-emerald-50 border border-emerald-100">
                      <div>
                    ...
                    <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-3.5 shadow-xs">
                    ...
                    {/* Available Alternatives Suggestion */}
                    ...
                    <div className="flex flex-col gap-2 pt-1">
                      <button
                        type="button"
                        onClick={() => setStep(2)}
                        className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-sm transition-all duration-200 flex items-center justify-center shadow-md hover:shadow-lg cursor-pointer"
                        style={btnStyle}
                      >
                      ...
                      <button
                        type="button"
                        disabled
                        className="w-full bg-gray-100 border border-gray-200 text-gray-400 font-semibold rounded-xl text-sm flex items-center justify-center cursor-not-allowed opacity-60"
                        style={btnStyle}
