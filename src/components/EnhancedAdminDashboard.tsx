...
export function EnhancedAdminDashboard() {
  ...
  const [email, setEmail] = useState(() => {
    try {
      return localStorage.getItem('resti_admin_email') || '';
    } catch {
      return '';
    }
  });
  ...
  const [rememberMe, setRememberMe] = useState(() => {
    try {
      return localStorage.getItem('resti_admin_remember') === 'true';
    } catch {
      return false;
    }
  });
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.session && data.user) {
        try {
          if (rememberMe) {
            localStorage.setItem('resti_admin_remember', 'true');
            localStorage.setItem('resti_admin_email', email);
          } else {
            localStorage.removeItem('resti_admin_remember');
            localStorage.removeItem('resti_admin_email');
          }
        } catch {}
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen flex bg-gradient-to-br from-slate-50 via-white to-slate-100">
        ...
        <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-emerald-950 via-emerald-800 to-teal-800 flex-col items-center justify-center p-12 relative overflow-hidden">
          ...
          <div className="absolute top-1/2 left-1/4 w-32 h-32 bg-emerald-400/10 rounded-full blur-2xl pointer-events-none" />
          
          <div className="relative z-10 text-center max-w-lg flex flex-col items-center">
            {/* Prominent Logo on high-contrast white card */}
            <div className="bg-white rounded-3xl p-6 mb-6 shadow-2xl border border-white/30 inline-flex items-center justify-center hover:scale-105 transition-transform duration-500">
              <img 
                src={loginLogo || '/logo.png'} 
                alt={loginSiteName || 'RESTI CBO Logo'} 
                className="h-28 w-auto max-w-[240px] object-contain drop-shadow-sm" 
              />
            </div>

            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-emerald-100 text-xs font-semibold tracking-wider uppercase mb-4">
              <ShieldCheck className="w-4 h-4 text-emerald-300" />
              <span>Official Management Console</span>
            </div>

            <h1 className="text-3xl lg:text-4xl font-extrabold text-white mb-2 tracking-tight">
              {loginSiteName || 'RESTI CBO'}
            </h1>
            <p className="text-emerald-100/95 text-base font-medium leading-relaxed mb-8 max-w-md">
              {loginTagline || 'Refugee Empowerment For Sustainable Transformation Initiative'}
            </p>

            {/* Impact stats */}
            <div className="grid grid-cols-3 gap-4 w-full mt-2">
              {loginStats.map(s => (
                <div key={s.label} className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/15 shadow-lg hover:bg-white/15 transition-all duration-300">
            ...
            {/* Regional Affiliation / Certification Badge */}
            <div className="mt-8 flex items-center justify-center gap-2 text-xs text-emerald-200/80">
              <MapPin className="w-3.5 h-3.5 text-emerald-300 shrink-0" />
              <span>Kiryandongo District, Uganda</span>
              <span className="opacity-40">•</span>
              <span>Certified & Regulated CBO</span>
            </div>
          </div>
        </div>

        {/* Right form panel */}
        <div className="w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-6 md:p-8 relative overflow-y-auto min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50/30">
          {/* Decorative glowing gradient orbs */}
          <div className="absolute -top-40 -right-40 w-96 h-96 bg-gradient-to-br from-emerald-100/30 to-teal-100/20 rounded-full blur-[80px] pointer-events-none" />
          <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-gradient-to-br from-teal-100/25 to-emerald-100/10 rounded-full blur-[80px] pointer-events-none" />
          
          <div 
            className="w-full max-w-md relative z-10 py-8 my-auto"
            style={{
          ...
          >
            ...
            {/* Main Login Card */}
            <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/60 border border-slate-100 p-6 sm:p-8 hover:shadow-2xl transition-all duration-300">
              {/* Prominent Logo & Identity Header at Top of Card */}
              ...
              {/* Contextual Action Title */}
              <div className="mb-6 pt-4 border-t border-slate-100">
                <h2 className="text-base sm:text-lg font-bold text-slate-800 tracking-tight">
                  {showForgotPassword ? 'Reset Password' : (isSignup ? 'Create Administrator Account' : 'Administrator Login')}
                </h2>
                <p className="text-slate-500 text-xs sm:text-sm font-normal mt-0.5 leading-relaxed">
                  {showForgotPassword 
                    ? 'Enter your registered email address to receive password recovery instructions' 
                    : (isSignup 
                        ? 'Fill in your details below to request administrator access' 
              ...
              {showForgotPassword ? (
                  ...
                  <div>
                    ...
                    <div className="relative rounded-xl shadow-xs group">
                      ...
                      <input
                        type="email"
                        value={resetEmail}
                        onChange={(e) => { setResetEmail(e.target.value); setAuthError(null); }}
                        placeholder="admin@example.com"
                        required
                        className="w-full pl-11 pr-4 py-2.5 sm:py-3 bg-slate-50/70 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 transition-all placeholder:text-slate-400 font-medium text-slate-800 text-sm focus:bg-white"
                      />
                    </div>
                  </div>
                  <Button
                    type="submit"
                    className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white py-3 rounded-xl shadow-md hover:shadow-lg transition-all font-semibold text-sm tracking-wide mt-6 disabled:opacity-60 disabled:cursor-not-allowed"
                    disabled={loading}
                  >
                    {loading ? (
                ...
                <form onSubmit={isSignup ? handleSignup : handleLogin} className="space-y-4">
                    ...
                    <div className="flex items-center justify-between text-xs pt-1">
                      <label className="flex items-center gap-2 cursor-pointer text-slate-600 select-none">
                        <input
                          ...
                          className="w-4 h-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 focus:ring-offset-0 transition-colors"
                        />
                  ...
                  <Button
                    type="submit"
                    className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white py-3 rounded-xl shadow-md hover:shadow-lg transition-all font-semibold text-sm tracking-wide mt-6 active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed"
                    disabled={loading}
                  >
                    {loading ? (
                      <div className="flex items-center justify-center gap-2">
                      ...
                      isSignup ? 'Create Administrator Account' : 'Sign In to Admin Portal'
                    )}
                  </Button>
                </form>
              )}
              ...
              <div className="mt-6 pt-5 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                {showForgotPassword ? (
                  <button
                    ...
                    className="text-emerald-600 hover:text-emerald-700 text-sm font-semibold transition-colors w-full text-center flex items-center justify-center gap-1.5"
                  >
                  ...
                  <button
                    ...
                    className="text-slate-600 hover:text-emerald-700 text-sm font-medium transition-colors w-full text-center"
                  >
                    {isSignup ? (
                      <>Already have an administrator account? <span className="font-semibold text-emerald-600 underline">Sign In</span></>
                    ) : (
                      <>Need an administrator account? <span className="font-semibold text-emerald-600 underline">Sign Up</span></>
                    )}
                  </button>
                )}
              </div>
            ...
            <div className="mt-6 text-center">
              ...
              >
                ...
                <span>Return to RESTI CBO Public Website</span>
              </a>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-4 h-16">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="bg-white rounded-xl p-1 border border-slate-700/60 shadow-sm flex items-center justify-center overflow-hidden h-[42px] w-[42px] shrink-0">
                <img src={loginLogo || '/logo.png'} alt="RESTI CBO Logo" className="h-full w-full object-contain shrink-0 block" />
              </div>
              <div className="hidden md:block">
                <h1 className="text-sm font-semibold text-white leading-tight">{loginSiteName || 'RESTI CBO'}</h1>
